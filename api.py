...
@cache.cached(query_string=True)
def search_podcasts():
    def resolve_mpsppl_to_mpre(browse_id: str):
        """Resolve an MPSPPL (podcast playlist) ID to MPRE (podcast episodes playlist)."""
        try:
            playlist_data = ytmusic.get_playlist(browse_id)
            # Try audioPlaylistId or relatedBrowseId
            resolved_id = playlist_data.get("audioPlaylistId") or playlist_data.get("relatedBrowseId")
            if resolved_id and resolved_id.startswith("MPRE"):
                return resolved_id
        except Exception as e:
            logger.debug(f"Failed to resolve MPSPPL {browse_id}: {str(e)}")
        return None
    ...
    try:
        ...
        if limit < 1 or page < 1 or page_size < 1:
            return jsonify({"error": "Invalid limit, page, or page_size parameter"}), 400

        # Search podcasts
        search_results = ytmusic.search(query, filter="podcasts", limit=limit * page_size)
        ...
        for result in search_results:
            ...
            resolved_browse_id = None

            # Resolve MPSPPL to MPRE
            if browse_id.startswith("MPSPPL"):
                resolved_browse_id = resolve_mpsppl_to_mpre(browse_id)

            podcast = {
                "title": result.get("title", ""),
                "browseId": browse_id,
                "resolvedId": resolved_browse_id if resolved_browse_id else None,
                "thumbnails": [t.get("url", "") for t in result.get("thumbnails", [])],
        ...
        return jsonify(
            {
                ...
                "count": len(paginated_podcasts),
                "total_count": len(podcasts),
                "page": page,
                "page_size": page_size,
                "total_pages": (len(podcasts) + page_size - 1) // page_size,
            }
        )
...
# Video search with enhanced filters
@app.route("/video/search", methods=["GET"])
@cache.cached(query_string=True)
def search_videos():
    try:
        query = request.args.get("q")
        duration_filter = request.args.get("duration", "any")  # short, medium, long, any
        upload_date = request.args.get("upload_date", "any")   # hour, day, week, month, year, any
        page = request.args.get("page", 1, type=int)
        page_size = request.args.get("page_size", 20, type=int)
        ...
        # Enhanced video search with filters
...
@app.route("/podcast/<browse_id>/episodes", methods=["GET"])
@cache.cached(query_string=True)
def get_podcast_episodes(browse_id):
    try:
        ...
        podcast_data = ytmusic.get_album(browse_id)  # Podcasts use album endpoint
        
        episodes = []
        for track in podcast_data.get("tracks", []):
            episode = {
                "title": track.get("title", ""),
                ...
                "episode_number": track.get("index", 0),
                "publish_date": track.get("year", "")
            }
        ...
        return jsonify({
            "podcast_id": browse_id,
            "title": podcast_data.get("title", ""),
            "description": podcast_data.get("description", ""),
            "episodes": paginated_episodes,
            "total_episodes": len(episodes),
            "page": page,
            "total_pages": (len(episodes) + limit - 1) // limit
        })
        
    except Exception as e:
        logger.error(f"Podcast episodes error: {str(e)}")
...
@app.route("/podcast/<episode_id>/play", methods=["GET"])
def play_podcast(episode_id):
    """
    Play a podcast episode by ID (audio only).
    """
    try:
        from pytube import YouTube
        yt = YouTube(f"https://www.youtube.com/watch?v={episode_id}")

        # fetch best audio stream
