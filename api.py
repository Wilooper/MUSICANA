...
def search_podcasts():
    try:
        ...
        # Search for podcasts using YouTube's podcast filter
        ...
        paginated_results = podcasts[start:end]
        ...
        return jsonify({"error": f"Podcast search failed: {str(e)}"}), 500
...
# Video download with format options
...
def download_video(video_id):
    try:
        ...
        # Start async download with video-specific options
        job_id = start_async_video_download(video_id, quality, format_type, audio_only)
        ...
        return jsonify({"error": f"Failed to start video download: {str(e)}"}), 500

def start_async_video_download(video_id, quality, format_type, audio_only):
    """Enhanced download function for videos with quality options"""
    import uuid
    import threading
    ...
    def download_worker():
        try:
            ...
            if audio_only:
                ...
                filename = f"{job_id}_audio.mp4"
            else:
                stream = yt.streams.filter(resolution=quality, file_extension=format_type).first()
                if not stream:
                    stream = yt.streams.filter(file_extension=format_type).order_by("resolution").desc().first()
                filename = f"{job_id}_video.{format_type}"
            
            if stream:
                stream.download(output_path="downloads", filename=filename)
                # Update job status to completed
                update_download_status(job_id, "completed", 100, filename)
            else:
                update_download_status(job_id, "failed", 0, None)
    ...
    update_download_status(job_id, "processing", 0, None)
